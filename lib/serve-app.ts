...
// Shared entry logic for every Whop-routed path (app path, dashboard path, and
// the root fallback). Whop's proxy injects the `x-whop-user-token` header on
// whichever path it loads inside its iframe, so all paths verify access the
// same way. Verified, paying members get the real TradeSafe app; everyone else
// gets a members-only / diagnostic screen.
export async function serveTradeSafe(headerList: Headers): Promise<Response> {
  const result = await resolveAccess(headerList)
