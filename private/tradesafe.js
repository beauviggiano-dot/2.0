...
const LS = {
  ...
  activeAccount: 'rl_active_account',
...
const legacyBalance = load(LS.account, 10000);
const initialAccounts = load(LS.accounts, null) || [{id:'default', title:'Primary Account', category:'live', startingBalance:legacyBalance, withdrawals:0, payouts:0, payoutTotal:0, blown:false}];
let state = {
  account: legacyBalance,
  accounts: initialAccounts,
  activeAccount: load(LS.activeAccount, 'all'),
...
function renderAccountList(){ const el=document.getElementById('accountList'); if(!el)return; const all=`<button type="button" class="text-left border ${state.activeAccount==='all'?'border-gold':'border-line'} rounded-xl p-3 bg-panel2" data-account="all"><div class="font-medium">All accounts</div><div class="text-xs text-muted mt-1">Combined balance · <span class="sensitive">${fmtMoney(currentBalance())}</span></div></button>`; el.innerHTML=all+state.accounts.map(a=>`<div class="flex items-center gap-2 border ${state.activeAccount===a.id?'border-gold':'border-line'} rounded-xl p-3 ${a.blown?'opacity-45':''}"><button type="button" class="text-left flex-1 min-w-0" data-account="${a.id}"><div class="flex justify-between gap-2"><span class="font-medium truncate">${a.title}</span><span class="text-[10px] uppercase text-faint">${a.category}</span></div><div class="text-xs text-muted mt-1">${a.blown?'Blown · ':''}<span class="sensitive">${fmtMoney(accountBalance(a))}</span></div></button><button type="button" class="btn-ghost text-xs shrink-0" data-edit-account="${a.id}">Edit</button></div>`).join(''); el.querySelectorAll('[data-account]').forEach(b=>b.addEventListener('click',()=>{state.activeAccount=b.dataset.account;save(LS.activeAccount,state.activeAccount);refreshAccountDisplays();calcSizer();renderDashboard();renderJournal();renderTradeAnalyst();renderAccountList();editAccount(state.activeAccount!=='all'?state.accounts.find(a=>a.id===state.activeAccount):null);})); el.querySelectorAll('[data-edit-account]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();editAccount(state.accounts.find(a=>a.id===b.dataset.editAccount));})); }
...
function updateAccountModalSummary(){ const start=parseFloat(document.getElementById('accountBalanceInput').value)||0; const adjustment=parseFloat(document.getElementById('accountAdjustmentInput').value)||0; const editing=state.editingAccountId ? state.accounts.find(a=>a.id===state.editingAccountId) : null; const pnl=editing ? state.trades.filter(t=>tradeBelongsToAccount(t,editing.id)).reduce((s,t)=>s+(t.pnl||0),0) : 0; document.getElementById('accountRealizedPnl').textContent=(pnl>=0?'+':'')+fmtMoney(pnl); document.getElementById('accountCurrentBalance').textContent=fmtMoney(start+adjustment+pnl); }
document.getElementById('openAccountBtn').addEventListener('click',openAccountModal); document.getElementById('mobileAccountBtn').addEventListener('click',openAccountModal); document.getElementById('accountCategoryInput').addEventListener('change',updateAccountCategoryFields); document.getElementById('accountBalanceInput').addEventListener('input',updateAccountModalSummary); document.getElementById('accountAdjustmentInput').addEventListener('input',updateAccountModalSummary); document.getElementById('accountCancelBtn').addEventListener('click',()=>document.getElementById('accountModal').classList.add('hidden')); document.getElementById('accountNewBtn').addEventListener('click',()=>editAccount(null));
document.getElementById('accountSaveBtn').addEventListener('click',()=>{ const v=parseFloat(document.getElementById('accountBalanceInput').value); const title=document.getElementById('accountTitleInput').value.trim(); if(!title||isNaN(v)||v<0){toast('Enter an account title and valid balance.');return;} const adjustment=parseFloat(document.getElementById('accountAdjustmentInput').value)||0; const data={id:state.editingAccountId||uid(),title,category:document.getElementById('accountCategoryInput').value,startingBalance:v+adjustment,withdrawals:parseFloat(document.getElementById('accountWithdrawalsInput').value)||0,payouts:parseInt(document.getElementById('accountPayoutsInput').value,10)||0,payoutTotal:parseFloat(document.getElementById('accountPayoutTotalInput').value)||0,blown:document.getElementById('accountBlownInput').checked}; const i=state.accounts.findIndex(a=>a.id===data.id); if(i>=0)state.accounts[i]=data;else state.accounts.push(data); state.activeAccount=data.id; save(LS.accounts,state.accounts);save(LS.activeAccount,state.activeAccount);refreshAccountDisplays();calcSizer();renderDashboard();renderAccountList();toast('Account saved.'); });
document.getElementById('accountDeleteBtn').addEventListener('click',()=>{if(!state.editingAccountId||state.accounts.length<=1){toast('Keep at least one account.');return;} if(!confirm('Delete this account? Its journal trades will remain in the combined view.'))return; state.accounts=state.accounts.filter(a=>a.id!==state.editingAccountId);state.activeAccount='all';save(LS.accounts,state.accounts);save(LS.activeAccount,'all');refreshAccountDisplays();renderDashboard();renderAccountList();editAccount(null);});
...
document.getElementById('tradeSaveBtn').addEventListener('click', ()=>{
  ...
  const data = {
    ...
    accountId: state.editingTradeId ? (state.trades.find(t=>t.id===state.editingTradeId)?.accountId || state.accounts[0]?.id) : (state.activeAccount==='all' ? state.accounts[0]?.id : state.activeAccount),
...
function appendAnalystMessage(role,text){ const wrap=document.getElementById('analystMessages'); const el=document.createElement('div'); el.className='analyst-message rounded-xl border border-line p-3 text-sm text-foreground whitespace-pre-wrap '+(role==='user'?'bg-panel2 ml-6':'bg-background mr-6'); el.textContent=text; wrap.appendChild(el); wrap.scrollTop=wrap.scrollHeight; return el; }
function renderAnalystGrades(text){
  const box=document.getElementById('analystGrades'), list=document.getElementById('analystGradeList'); if(!box||!list)return;
  const grades=[...String(text||'').matchAll(/GRADE:\s*Trade\s*(\d+)\s*=\s*(A\+|A|B|C|D|F)\b/gi)].map(m=>({trade:m[1],grade:m[2].toUpperCase()}));
  if(!grades.length){ box.classList.add('hidden'); return; }
  const tone={"A+":"text-profit border-profit","A":"text-profit border-profit","B":"text-gold border-gold","C":"text-gold border-gold","D":"text-loss border-loss","F":"text-loss border-loss"};
  list.innerHTML=grades.map(g=>`<span class="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${tone[g.grade]||'text-muted border-line'}"><span class="text-xs text-faint">Trade ${analystEscape(g.trade)}</span><strong class="font-display text-lg">${analystEscape(g.grade)}</strong></span>`).join('');
...
function bindTradeAnalyst(){
  ...
  document.getElementById('analystClearWorkspaceBtn').addEventListener('click',()=>{ if(!confirm('Clear selected trades and all AI analysis from this workspace?')) return; state.analystSelected=[]; state.analystHistory=[]; save(LS.analystSelected,[]); save(LS.analystHistory,[]); const messages=document.getElementById('analystMessages'); if(messages) messages.innerHTML='<div class="text-sm text-muted rounded-xl border border-dashed border-line p-4">Start by saving your strategy profile, selecting a journal trade, and describing what you saw.</div>'; const grades=document.getElementById('analystGrades'); if(grades) grades.classList.add('hidden'); renderTradeAnalyst(); toast('Workspace and AI analysis cleared.'); });
