...
function accountTrades(){ const ids=new Set(activeAccounts().map(a=>a.id)); return state.trades.filter(t=>!t.accountId || ids.has(t.accountId)); }
...
function updateAccountModalSummary(){ const start=parseFloat(document.getElementById('accountBalanceInput').value)||0; const pnl=realizedPnl(); document.getElementById('accountRealizedPnl').textContent=(pnl>=0?'+':'')+fmtMoney(pnl); document.getElementById('accountCurrentBalance').textContent=fmtMoney(start+pnl); }
document.getElementById('openAccountBtn').addEventListener('click',openAccountModal); document.getElementById('mobileAccountBtn').addEventListener('click',openAccountModal); document.getElementById('accountCategoryInput').addEventListener('change',updateAccountCategoryFields); document.getElementById('accountBalanceInput').addEventListener('input',updateAccountModalSummary); document.getElementById('accountCancelBtn').addEventListener('click',()=>document.getElementById('accountModal').classList.add('hidden')); document.getElementById('accountNewBtn').addEventListener('click',()=>editAccount(null));
document.getElementById('accountSaveBtn').addEventListener('click',()=>{ const v=parseFloat(document.getElementById('accountBalanceInput').value); const title=document.getElementById('accountTitleInput').value.trim(); if(!title||isNaN(v)||v<0){toast('Enter an account title and valid balance.');return;} const data={id:state.editingAccountId||uid(),title,category:document.getElementById('accountCategoryInput').value,startingBalance:v,withdrawals:parseFloat(document.getElementById('accountWithdrawalsInput').value)||0,payouts:parseInt(document.getElementById('accountPayoutsInput').value,10)||0,payoutTotal:parseFloat(document.getElementById('accountPayoutTotalInput').value)||0,blown:document.getElementById('accountBlownInput').checked}; const i=state.accounts.findIndex(a=>a.id===data.id); if(i>=0)state.accounts[i]=data;else state.accounts.push(data); state.activeAccount=data.id; save(LS.accounts,state.accounts);save(LS.activeAccount,state.activeAccount);refreshAccountDisplays();calcSizer();renderDashboard();renderAccountList();toast('Account saved.'); });
