...
function accountTrades(){ const ids=new Set(activeAccounts().map(a=>a.id)); return state.activeAccount==='all' ? state.trades : state.trades.filter(t=>t.accountId===state.activeAccount); }
function realizedPnl(){ return accountTrades().reduce((s,t)=>s+(t.pnl||0),0); }
function currentBalance(){ return activeAccounts().reduce((s,a)=>s+a.startingBalance,0)+realizedPnl(); }
function accountBalance(a){ return a.startingBalance + state.trades.filter(t=>!t.accountId||t.accountId===a.id).reduce((s,t)=>s+(t.pnl||0),0); }
...
function renderAccountList(){ const el=document.getElementById('accountList'); if(!el)return; const all=`<button type="button" class="text-left border ${state.activeAccount==='all'?'border-gold':'border-line'} rounded-xl p-3 bg-panel2" data-account="all"><div class="font-medium">All accounts</div><div class="text-xs text-muted mt-1">Combined balance · ${fmtMoney(currentBalance())}</div></button>`; el.innerHTML=all+state.accounts.map(a=>`<button type="button" class="text-left border ${state.activeAccount===a.id?'border-gold':'border-line'} rounded-xl p-3 ${a.blown?'opacity-45':''}" data-account="${a.id}"><div class="flex justify-between gap-2"><span class="font-medium">${a.title}</span><span class="text-[10px] uppercase text-faint">${a.category}</span></div><div class="text-xs text-muted mt-1">${a.blown?'Blown · ':''}${fmtMoney(accountBalance(a))}</div></button>`).join(''); el.querySelectorAll('[data-account]').forEach(b=>b.addEventListener('click',()=>{state.activeAccount=b.dataset.account;save(LS.activeAccount,state.activeAccount);refreshAccountDisplays();calcSizer();renderDashboard();renderAccountList();})); }
